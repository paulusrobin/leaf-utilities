...
func (DeletedAt) DeleteClauses(f *schema.Field) []clause.Interface {
	...
	if v := settings["DELETEDBYFIELD"]; v != "" { // DeletedByField
		softDeleteClause.DeleteByField = f.Schema.LookUpField(v)
	}
...
func (sd SoftDeleteDeleteClause) ModifyStatement(stmt *gorm.Statement) {
	if stmt.SQL.Len() == 0 && !stmt.Statement.Unscoped {
		...
		set := clause.Set{}
		if sd.Flag {
			set = append(set, clause.Assignment{Column: clause.Column{Name: sd.Field.DBName}, Value: 1})
			...
			set = append(set, clause.Assignment{Column: clause.Column{Name: sd.Field.DBName}, Value: curUnix})
			stmt.SetColumn(sd.Field.DBName, curUnix, true)
		}

		if sd.DeleteByField != nil {
			mandatory := leafMandatory.FromContext(stmt.Context)
			set = append(set, clause.Assignment{Column: clause.Column{Name: sd.DeleteByField.DBName}, Value: mandatory.User().Email()})
			stmt.SetColumn(sd.DeleteByField.DBName, mandatory.User().Email(), true)
		}
